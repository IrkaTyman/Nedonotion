...
import { InputProps, getFlexContainerStyleClasses } from '@shared/ui';
...
export const FileInput: FC<Props> = typedMemo(function FileInput({
    ...
    const onChange: ChangeEventHandler<HTMLInputElement> = useCallback(event => {
        ...
        if (file && file.size > maxSizeByMb * 1024 * 1024 * 8) {
            changeError(t('core_warning_file_size', { size: maxSizeByMb }));
        } else if (file && fileType && !acceptType?.includes(`.${fileType}`)) {
            changeError(`${t('core_warning_formats_in_dnd')} ${acceptType.join(', ')}`);
        } else {
            changeError(null);
            ...
            >
...

                {
                    fileUrl
                        ...
                        >
                            {error ?? placeholder ?? t('core_click_or_drag_files')}
                        </p>
                }
            </label>
